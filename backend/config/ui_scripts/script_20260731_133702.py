        ...
        async with async_playwright() as p:
            ...
            for loc_expr in [
                ...
                try:
                    locator = loc_expr()
                    await locator.wait_for(state="visible", timeout=5000)
                    await locator.fill("JS_258369")
                    input_done = True
                    break
                except Exception:
                    continue
            if not input_done:
                await page.locator("#form_item_password").fill("JS_258369", force=True)
            await page.wait_for_timeout(STEP_DELAY)
            ...
            # 截图保存
            await page.screenshot(path="E:/trae_work/autoProject/backend/config/ui_results/result_20260731_133702/test_result.png")
            await browser.close()
            print("✓ 测试执行完成")
